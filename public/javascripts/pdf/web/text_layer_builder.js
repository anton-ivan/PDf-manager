...
var TextLayerBuilder = function textLayerBuilder(options) {
  ...
  this.renderLayer = function textLayerBuilderRenderLayer() {
    ...
    for (var i = 0, ii = textDivs.length; i < ii; i++) {
      var id_dex=(this.pageIdx+1)+"x"+i;
      var textDiv = textDivs[i];
      if ('isWhitespace' in textDiv.dataset) {
        continue;
      }

      textDiv.setAttribute("target_index",id_dex);
      if (TASK_ASSOC_ARY[id_dex] && TASK_ASSOC_ARY[id_dex]!="")
      {
          textDiv.className  = textDiv.className + " highlighted_div";
          textDiv.setAttribute("task_id",TASK_ASSOC_ARY[id_dex]);
      }
